...
async function runTest() {
  ...
  console.log('\n3️⃣ Purchasing 2 cups of Valencia Orange Juice (productId = 4)...');
  await httpRequest(`${API_BASE}/pos/checkout`, {
    method: 'POST',
    body: JSON.stringify({ items: [{ productId: 4, quantity: 2, cupSizeMl: 250 }], paymentMethod: 'CASH' })
  });

  console.log('\n4️⃣ DB Prices after buying 2 cups of Orange:');
  console.log(queryPgSql("SELECT id, name, current_cup_price FROM products WHERE id IN (1,2,3,4,5,6,7,23) ORDER BY id;"));

  console.log('\n5️⃣ Resetting prices again...');
  await httpRequest(`${API_BASE}/pricing/reset-all`, {
    method: 'POST',
    headers: { 'Authorization': `Bearer ${token}` }
  });
  ...
  console.log('\n6️⃣ Purchasing 1 cup of Zesty Lemon Juice (productId = 2)...');
  await httpRequest(`${API_BASE}/pos/checkout`, {
    method: 'POST',
    body: JSON.stringify({ items: [{ productId: 2, quantity: 1, cupSizeMl: 250 }], paymentMethod: 'CASH' })
  });

  console.log('\n7️⃣ DB Prices after buying 1 cup of Lemon:');
  console.log(queryPgSql("SELECT id, name, current_cup_price FROM products WHERE id IN (1,2,3,4,5,6,7,23) ORDER BY id;"));
}
